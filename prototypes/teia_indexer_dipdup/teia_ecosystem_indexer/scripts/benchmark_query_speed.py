...
def get_top_wallets(db_path: Path, limit: int = 5) -> list[str]:
    ...
    if 'token' in tables:
        ...
        try:
            res = conn.execute(
                'SELECT creator_address FROM token WHERE creator_address IS NOT NULL GROUP BY creator_address ORDER BY COUNT(*) DESC LIMIT ?',
                (limit,),
            ).fetchall()
...
def run_benchmark():
    ...
    print('--- Benchmarking Address Interning ---')
